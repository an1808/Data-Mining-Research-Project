            ...
            if list1 != list2:
                while count < length - 1:
                    ...
                    if list1[length - 1] < list2[length - 1]:
                        ...
                        if not Infrequent_subset(c, Lk_1, k):
                            Ck.append(c)
                            c = []
    return Ck
...
"""Infrequent_subsets function to determine if pruning is required to remove unfruitful candidates (c) using the Apriori property, with prior knowledge of frequent (k-1)-itemset (Lk_1)"""


def Infrequent_subset(c, Lk_1, k):
    list = []
